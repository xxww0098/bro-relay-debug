...
CLI 已随 `skills/bro-connect/scripts/runtime` 内置；代理电脑只需 Node.js 22.16+，无需安装 npm 软件包。
可直接复制整个 `skills/bro-connect` 目录到代理的技能目录。源码更新后执行
`npm run skill:install` 同步并安装运行文件；移动或移除源码目录不影响已安装技能。
...
操作直接发生在当前标签页：指针、目标框和动作标签画在页面本身上，用户可以实时看到。
截图前会先撤掉提示，代理证据里不含指针层。代理结束工作时发出 `release`，指针和提示一起消失；
`disconnect` 只忘本地凭证，并不结束浏览器上的控制。
判断结果应读取操作返回值和新的页面状态。点击会在限定时间内等待目标可用并停止移动；
超时后应重新观察页面，不要盲目重放操作。
关闭扩展面板的远程控制开关会取消正在执行和排队的任务，并关闭远程控制；再次启用沿用原驱动 ID。
动作批次和脚本直接在当前标签页执行，不再打开预览页；只读查询不会切换页面。
后台标签页默认拒绝可视化输入（`needs_foreground`），需要时用 `focus` 或 `allowFocus: true`。
代理运行异常时，在扩展弹窗关闭“远程控制”即可取消运行中和排队的任务。
