...
navigate URL | click TARGET | fill TARGET TEXT
...
async function selectTab(browser, requested) {
  ...
  if (requested) {
    ...
    if (!found) throw new RelayError({ code: 'tab_not_found', message: 'Tab is no longer available; list tabs again', status: 404 });
    return found;
  }
...
async function main() {
  ...
  if (['state', 'read', 'observe'].includes(command)) {
    ...
    return output(command === 'state' ? { ok: true, tab, observation } : observation);
  }
  if (command === 'find') {
  ...
  if (command === 'actions') {
    ...
    return output(await browser.request('POST', '/api/actions', { ...body, tabId }));
  }
  ...
  switch (command) {
    ...
    case 'click': action = { type: 'click', target: required(args[0], 'Target') }; break;
    case 'fill':
  ...
  return output(await browser.request('POST', '/api/actions', { tabId, actions: [action] }));
}
